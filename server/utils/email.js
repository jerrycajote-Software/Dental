...
const sendEmail = async ({ to, subject, html }) => {
  ...
  const senderEmail = process.env.SMTP_USER || process.env.ADMIN_EMAIL;
  if (!senderEmail) {
    throw new Error('No sender email found. Set SMTP_USER or ADMIN_EMAIL in environment variables.');
  }

  const payload = {
    sender: { name: 'Dental CarePlus', email: senderEmail },
...
const sendVerificationEmail = async (to, name, token) => {
  ...
  try {
    const data = await sendEmail({ to, subject: 'Verify Your Email - Dental CarePlus', html });
    console.log(`✉️  Verification email sent to: ${to} (messageId: ${data?.messageId})`);
    return data;
  } catch (error) {
...
const sendWalkinVerificationEmail = async (to, name, token, tempPassword) => {
  ...
  try {
    ...
    console.log(`✉️  Walk-in credentials email sent to: ${to} (messageId: ${data?.messageId})`);
    return data;
  } catch (error) {
...
const sendPasswordResetEmail = async (to, resetLink) => {
  ...
  try {
    const data = await sendEmail({ to, subject: 'Reset Your Password - Dental CarePlus', html });
    console.log(`✉️  Password reset email sent to: ${to} (messageId: ${data?.messageId})`);
  } catch (error) {
...
const sendPatientAppointmentNotification = async (patientEmail, patientName, appointmentDetails) => {
  ...
  try {
    ...
    console.log(`✉️  Appointment confirmation sent to ${patientName} (${patientEmail}) (messageId: ${data?.messageId})`);
  } catch (error) {

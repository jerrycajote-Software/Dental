...
/**
 * Global Loader component for asynchronous wait-states.
 * @param {boolean} fullScreen - Renders as a full-screen fixed overlay.
 * @param {boolean} overlay - Renders as an absolute overlay within a relative parent.
 * @param {string} text - Optional text to display below the loader.
 */
const Loader = ({ fullScreen = false, overlay = false, text = '' }) => {
  const content = (
    <div className="flex flex-col items-center justify-center">
      <div className="bg-slate-900/90 p-8 rounded-[2rem] shadow-2xl flex flex-col items-center justify-center border border-slate-700/50 backdrop-blur-md">
        <StyledWrapper>
        ...
        {text && <p className="mt-6 text-sm font-black tracking-widest text-white uppercase animate-pulse">{text}</p>}
      </div>
    </div>
  );
  ...
  if (fullScreen) {
    return (
      <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-slate-50/50 backdrop-blur-sm">
        {content}
      </div>
    );
  }
...
const StyledWrapper = styled.div`
  .loader {
    --dur: 2s;
    /* Default hue set to a blue tone matching the app primary color (#1089d3) */
    --hue: 200;
    display: block;
    margin: auto;
    width: 3.5rem;
